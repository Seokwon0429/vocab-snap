...
- 정밀 인식 모드에서 두 가지 대비 보정 결과를 비교해 더 안정적인 결과 선택
...
- 기본값인 **정밀 인식**은 같은 사진을 균형 대비와 강한 대비로 각각 전처리한 뒤, 한 개의 Tesseract Worker에서 순서대로 인식합니다.
- 페이지 신뢰도, 단어별 신뢰도, 인식 문자량을 함께 비교해 결과 전체 중 하나를 선택합니다. 서로 다른 결과의 문장을 임의로 섞지 않습니다.
